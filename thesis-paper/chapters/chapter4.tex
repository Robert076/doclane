...
\par The backend runs as a Deployment in Kubernetes with 2 replicas by default. Each replica is a container running the Go binary that serves the chi HTTP router on port 8080. The same code that previously ran on Lambda now runs as a regular HTTP server in a container, with no code changes needed for the migration.
...
\par The backend generates presigned URLs for uploads and downloads. This means clients upload directly to S3 using a temporary URL that expires after a set time, without the file passing through the backend. This reduces load on the backend and leverages S3's scalability.
...
\par This split allows tearing down the expensive compute resources overnight while keeping the cheap persistent resources alive. A full environment can be recreated from scratch with a single command.
