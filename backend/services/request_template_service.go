...
func (s *RequestTemplateService) AddRequestTemplateWithDocuments(
	...
	// Upload all S3 files first, before touching the DB.
	// Track uploaded keys so we can roll them back if anything fails.
	...
	for i, doc := range docs {
		...
		s3Key := s.fileStorage.GenerateExampleS3Key(doc.ExampleFileName)
		result, err := s.fileStorage.UploadFile(ctx, s3Key, doc.ExampleFile, doc.ExampleMimeType)
		if err != nil {
			rollbackS3()
			return nil, errors.ErrBadGateway{Msg: fmt.Sprintf("Failed to upload example file %d to S3: %v", i, err)}
		}
	...
	// Build a lookup from doc index -> upload info
	...
	if err != nil {
		// DB failed — roll back all S3 uploads
		rollbackS3()
